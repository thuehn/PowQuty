...
static void *retrival_thread_run(void* param);
...
void handle_data_message(int read_size)
{
	...
	curr_idx = get_unsigned_short_val(current_frame + 4);
...
void join_retrieval()
{
	...
	if (raw_print)
		raw_dump_join();
	pthread_join(retrival_thread, NULL);
